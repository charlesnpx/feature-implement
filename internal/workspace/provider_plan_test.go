...
func TestPlanExternalProviderMergeAndRemoteDeleteCommands(t *testing.T) {
	t.Run("merge", func(t *testing.T) {
		...
		if !strings.Contains(planned.Plan.ProviderCommand, "cd "+attempt.Worktree+" && gh pr view 35 --json headRefOid,baseRefOid") ||
			!strings.Contains(planned.Plan.ProviderCommand, "head-sha base-sha") ||
			!strings.Contains(planned.Plan.ProviderCommand, "gh pr merge 35 --merge --match-head-commit head-sha") ||
			!strings.Contains(planned.Plan.ProviderCommand, "--intent "+planned.Intent.IntentID+" --status failed_before_side_effect") ||
			!strings.Contains(planned.Plan.ProviderCommand, "--intent "+planned.Intent.IntentID+" --status ambiguous") {
	...
	t.Run("remote delete", func(t *testing.T) {
		...
		approval := grantExternalIntentApprovalForTest(t, ready.Fixture.Dir, ready.Claim.MergeUnitID, ready.Attempt.AttemptID, ready.Claim.AgentID, ready.Claim.LeaseID, ExternalActionRemoteDelete, ready.Attempt.Branch, "", ready.HeadSHA, ready.BaseSHA, "2026-01-02T15:12:00Z")
		planned, err := PlanExternalProviderCommand(ExternalProviderPlanOptions{
		...
		if !strings.Contains(planned.Plan.ProviderCommand, "ls-remote upstream refs/heads/"+ready.Attempt.Branch) ||
			!strings.Contains(planned.Plan.ProviderCommand, "--force-with-lease=refs/heads/"+ready.Attempt.Branch+":"+ready.HeadSHA) ||
			!strings.Contains(planned.Plan.ProviderCommand, ":refs/heads/"+ready.Attempt.Branch) ||
			!strings.Contains(planned.Plan.ProviderCommand, "--intent "+planned.Intent.IntentID+" --status failed_before_side_effect") ||
			!strings.Contains(planned.Plan.ProviderCommand, "--intent "+planned.Intent.IntentID+" --status ambiguous") {
...
func TestPlanExternalProviderRemoteDeleteRequiresMergeEvidence(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "requires accepted merge external intent evidence") {
		t.Fatalf("PlanExternalProviderCommand remote-delete error = %v", err)
	}
}

func TestPlanExternalProviderRemoteDeleteRequiresMatchingMergeEvidence(t *testing.T) {
	ready := newQueueReadyAttemptFixture(t)
	if _, err := queueMergeReadyAttempt(t, ready, "", ready.Attempt.Branch, "2026-01-02T15:09:00Z"); err != nil {
		t.Fatalf("queue: %v", err)
	}
