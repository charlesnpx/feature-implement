...
func StartAttempt(opts AttemptStartOptions) (AttemptResult, error) {
	...
	commands := attemptWorktreeCommands(state.View.Repo, branch, worktree, opts.BaseSHA)
...
func attemptWorktreeCommands(repo string, branch string, worktree string, baseSHA string) []string {
	return []string{fmt.Sprintf("git -C %s worktree add -b %s %s %s", shellQuote(repo), shellQuote(branch), shellQuote(worktree), shellQuote(baseSHA))}
}
