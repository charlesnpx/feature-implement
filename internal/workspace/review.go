...
func (state ReviewState) InfrastructureRetriesUsed() uint16 {
	...
	for _, round := range state.rounds {
		for _, result := range round.attempts {
			...
			// invocation is a retry, regardless of whether that retry eventually
			// succeeds or reports another infrastructure failure.
			if result.request.invocation > 1 {
				used++
			}
		}
	}
...
func (state ReviewState) NextRequest() (ReviewRequest, bool, error) {
	...
	invocation := uint16(1)
	for _, result := range round.attempts {
		if result.request.profileOrdinal == uint16(profileIndex+1) {
			invocation++
		}
	}
...
func NewRecordReviewResult(
	...
	if round == 0 || profileOrdinal == 0 || invocation == 0 || err != nil ||
		result.digest != DigestBytes(canonical) || receiptDigest.IsZero() {
	...
	return RecordReviewResult{
		round: round, profileOrdinal: profileOrdinal, invocation: invocation,
...
func NewApplyReviewFix(
	ordinal uint16, priorHead, priorTree, head, tree GitObjectID,
	...
	if ordinal == 0 || priorHead.IsZero() || priorTree.IsZero() || head.IsZero() || tree.IsZero() ||
		...
		priorHead.Algorithm() != head.Algorithm() || head == priorHead || evidence.IsZero() || len(findings) == 0 {
	...
	for index, finding := range normalized {
		if finding.IsZero() || index > 0 && finding == normalized[index-1] {
			return ApplyReviewFix{}, fmt.Errorf("review fix finding identities must be nonzero and unique")
		}
	}
	return ApplyReviewFix{
		ordinal: ordinal, priorHead: priorHead, priorTree: priorTree,
...
func ReduceReview(current ReviewState, event ReviewEvent) (ReviewState, error) {
	...
	case RecordReviewResult:
		...
		if err != nil || !ok {
			return ReviewState{}, fmt.Errorf("review round has no pending profile")
		}
		if value.round != request.round || value.profileOrdinal != request.profileOrdinal ||
			...
			!value.result.isolation.Strict() {
			return ReviewState{}, fmt.Errorf("review result does not match the ordered exact-head request and strict sandbox")
		}
		if err := validateReviewInstancePolicy(current, request.profile, value.result.reviewerInstance); err != nil {
			return ReviewState{}, err
		}
		verified := VerifiedReviewResult{request: request, submission: cloneReviewResult(value.result), receiptDigest: value.receiptDigest}
	...
	case ApplyReviewFix:
		...
		if !round.Complete(len(current.loop.profiles)) || value.ordinal != uint16(len(current.fixes)+1) ||
			value.ordinal > current.loop.maxFixes || value.priorHead != current.head || value.priorTree != current.tree {
			return ReviewState{}, fmt.Errorf("review fix does not match durable round, budget, and current Git state")
		}
		...
		next.fixes = append(next.fixes, ReviewFixApplication{
			ordinal: value.ordinal, priorHead: value.priorHead, priorTree: value.priorTree,
...
func validateReviewInstancePolicy(state ReviewState, profile ReviewProfile, instance ID) error {
	...
	for _, round := range state.rounds {
		for _, result := range round.attempts {
			if result.request.profile.id == profile.id {
				prior = append(prior, result.submission.reviewerInstance)
			}
		}
	}
	if len(prior) == 0 {
...
func deriveReviewExhaustion(state ReviewState) *ReviewExhaustionDirective {
	...
	if len(state.rounds) != 0 {
		attempts := state.rounds[len(state.rounds)-1].attempts
		latestInfrastructureFailure = len(attempts) != 0 &&
			attempts[len(attempts)-1].submission.status == ReviewResultInfrastructureFailure
	}
