...
9. Ask a fresh Codex subagent to review the source bundle for missing implementation detail, invalid dependencies, unsafe grouping, policy weakening, unusable path constraints, missing authority inputs, and direct CLI incompatibility. Do not use a PR-review skill for this plan review.
10. Apply evidence-backed Critical and High findings, validate again, and ask a fresh reviewer to inspect the updated bundle. Repeat until a fresh review has no Critical or High findings. Apply worthwhile Medium and Low findings once.
