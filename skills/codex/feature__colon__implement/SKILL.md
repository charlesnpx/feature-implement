...
8. When using the generic Codex PR-review subagent, send the same reviewer the addressed findings, changed file list, and relevant local diff; ask whether the patch resolves their specific concerns. When using `pr:review:no-file`, apply selected fixes locally, then use a fresh no-file review after the fixes are pushed. Iterate locally until the reviewer confirms, a fresh no-file review returns no findings worth addressing, or you deliberately reject the concern with a concrete reason.
