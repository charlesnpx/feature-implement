...
3. Spawn a fresh Codex subagent for each fresh invocation. Give it the exact base-to-head branch diff and review request. The reviewer is read-only, uses ephemeral scratch, has no provider credentials, repository hooks, write network, external-write authority, or provider-broker capability.
