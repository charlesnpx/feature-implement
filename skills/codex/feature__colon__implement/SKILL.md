...
2. Use `feature status <plan-dir> --json` and `feature implement next <plan-dir> --json` to identify the next merge unit. Capture `story_progress_label` from the `next` result, for example `(Story 4/16)` or `(Stories 4-5/16)`, and include it in user-facing progress updates for the active work.
...
5. Push the implementation branch, open a PR with a clear title and description that includes the active `story_progress_label`, then record PR number/URL with `feature implement open-pr ... --write-state`.
6. For opened-PR reviews, check whether the active Codex Skills list includes `pr:review:no-file`. If it does, spawn a Codex subagent from the implementation worktree/repository path and instruct it to run `$pr:review:no-file <pr-number>` there to review the opened PR; include the active `story_progress_label` in the review context. If it does not, spawn the current generic Codex PR-review subagent. Use branch-diff review only when PR creation is not approved.
7. Run a PR review loop with a maximum of 10 fresh-review iterations. For each review with worthwhile findings, apply the selected fixes locally, run checks, close that reviewer subagent, then commit and push the accepted fixes. In workspace-managed execution, refresh the branch after the final implementation commit or review-fix commit, evaluate gates from that refreshed evidence, record tool-proven review, security, or test evidence with `feature workspace gate record`, and use those same base/head SHAs when entering the merge queue. Any later fix commit stales the refresh evidence; refresh again before gate evaluation, approval matching, or queue entry.
8. Spawn a fresh subagent to review the updated PR using the same skill-selection rule. Do not send addressed findings back to the previous reviewer for confirmation; the fresh review of the pushed branch is the confirmation mechanism.
9. Stop only when a fresh reviewer returns no findings worth addressing. If iteration 10 still has worthwhile findings, stop and report the remaining findings instead of merging.
