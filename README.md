...
4. Work only in the returned attempt worktree. Use `commit next` for configured commit steps; otherwise make ordinary local commits. Run the configured review loop and record only receipt-backed results for its exact head and tree.
